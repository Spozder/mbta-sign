...
class Prediction:
  ...
  def to_string(self, now=datetime.now(tzutc())):
    return "Prediction for {} line going {} (ID {}): {} ({} minutes from now)".format(
        ...
        self._pred_id.split("-")[1] if self._pred_id.split("-")[1] != "ADDED" else self._pred_id.split("-")[2],
...
class MBTAState:
  ...
  def apply_update(self, event):
    pred = parse_line(event.get_data())
    self._predictions_dict[pred['id']].update_arrival_time(pred['attributes']['arrival_time'])
  ...
  def get_next_two_predictions(self, route, direction_str, now=datetime.now(tzutc())):
    return sorted(
        filter(
            lambda pred: pred.get_arrival_time() > now and (pred.get_arrival_time() - now).seconds > 60 and pred.get_route() == route and str(pred.get_direction()) == direction_str,
            self._predictions_dict.values()
        )
    )[:2]

class EventType(Enum):
  RESET = lambda state, event: state.apply_reset(event)
  ADD = lambda state, event: state.apply_add(event)
  UPDATE = lambda state, event: state.apply_update(event)
  REMOVE = lambda state, event: state.apply_remove(event)
  ERROR = lambda state, event: raise_error_event()
...
def pred_stream(state, stop_id, direction_id, kill_event, state_update_event):
  pred_stream_r = requests.get(API_BASE + "/predictions?filter[stop]={}&filter[direction_id]={}".format(stop_id, direction_id), headers=headers, stream=True)
  ...
  while not kill_event.is_set():
    ...
    if line:
      if event_type_line:
        ...
        if type_str == 'keep-alive': continue
  ...
  while not kill_event.is_set():
    if mbta_state_update_event.is_set() or button_state_update_event.is_set():
      ...
      if button_state.get_held():
        ...
        predictions = mbta_state.get_next_two_predictions(color.value["mbta_string"], direction, now)
...
def got_verified_button(bd_addr, state, update_event):
  ...
  cc.on_button_single_or_double_click_or_hold = on_button_press_creator(state, update_event)
...
button_client.get_info(got_flic_server_info_factory(button_state, button_state_update_event))

button_handler_thread = threading.Thread(target=handle_button_events, args=(button_client,))

sign = Sign()

#state_printer_thread = threading.Thread(target=print_state, args=(mbta_state, kill_event, mbta_state_update_event))
sign_updater_thread = threading.Thread(target=update_sign, args=(sign, mbta_state, button_state, kill_event, mbta_state_update_event, button_state_update_event))

orange_north_thread = threading.Thread(target=pred_stream, args=(mbta_state, '70013', '1', kill_event, mbta_state_update_event))
orange_south_thread = threading.Thread(target=pred_stream, args=(mbta_state, '70012', '0', kill_event, mbta_state_update_event))
green_north_thread = threading.Thread(target=pred_stream, args=(mbta_state, '70242', '1', kill_event, mbta_state_update_event))
green_south_thread = threading.Thread(target=pred_stream, args=(mbta_state, '70241', '0', kill_event, mbta_state_update_event))
